...
public class SimulationGO : MonoBehaviour
{
    [SerializeField]
    private Bounds m_SimulationBounds;
    [SerializeField]
    private AgentGO m_AgentPrefab;
    [SerializeField]
    ...
    [SerializeField]
    private AgentSettingsGO m_HumanSettings;
    [SerializeField]
    private AgentSettingsGO m_ZombieSettings;

    private List<AgentGO> m_Agents = new();

    private static readonly ProfilerMarker s_AgentUpdateProfilerMarker = new ("SingleAgentUpdate");
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < m_HumanCount; ++i)
        {
            ...
            AgentGO newAgent = Instantiate(m_AgentPrefab, entityPosition, Quaternion.identity);
    ...
    private void InfectAgent(AgentGO agent)
    {
    ...
    private void TurnAgentToZombie(AgentGO agent)
    {
        agent.HealthState = AgentHealthState.Zombie;
        agent.Renderer.material = m_ZombieMaterial;
    }

    private void UpdateAgentDirection(AgentGO agent)
    {
        if (agent.MovementState == AgentMovementState.AimlessWalking)
        {
            if (Time.time > agent.NextDirectionChangeTime)
            {
                AgentSettingsGO settings = agent.HealthState == AgentHealthState.Zombie ? m_ZombieSettings : m_HumanSettings;
    ...
    private void UpdateAgentMovement(AgentGO agent)
    {
        AgentSettingsGO settings = agent.HealthState == AgentHealthState.Zombie ? m_ZombieSettings : m_HumanSettings;
    ...
    private static void ApplyDirection(AgentGO agent, float duration, float durationVariation, Vector3 direction)
    {
        agent.NextDirectionChangeTime = Time.time + duration + Random.value * durationVariation;
        agent.Direction = direction.normalized;
    }

    private void UpdateAgentHealth(AgentGO agent)
    {
    ...
    private void UpdateAgentFOV(AgentGO agent)
    {
    ...
    private void UpdateAgentAttack(AgentGO agent)
    {
